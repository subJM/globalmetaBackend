...
async function main() {
    ...
    const ContractFactory = await hre.ethers.getContractFactory("TEST3", wallet); // 지갑을 사용하여 계약 팩토리 생성
    const contract = await ContractFactory.deploy(address); // 계약 배포
    
    await contract.waitForDeployment();

    console.log("Contract deployed to runner:", contract.runner);
    console.log("Contract deployed to address:", contract.runner.address);
